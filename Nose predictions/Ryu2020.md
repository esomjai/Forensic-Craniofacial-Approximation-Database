...
[Ryu_soft_tissue.mrk.json](https://github.com/user-attachments/files/24859020/Ryu_soft_tissue.mrk.json)
...
def create_anatomical_planes():
    ...
    # ==============================================
    # Get auriculare landmarks to calculate midpoint
    au_L = None
    au_R = None
    O_L = None
    O_R = None
    ...
    for i in range(landmarksNode.GetNumberOfControlPoints()):
        name = landmarksNode.GetNthControlPointLabel(i)
        pos = np.zeros(3)
        landmarksNode.GetNthControlPointPosition(i, pos)
        if name == "au_L":
            au_L = pos
        elif name == "au_R":
            au_R = pos
    ...
    if au_L is None or au_R is None or O_L is None or O_R is None:
        raise ValueError("Missing required landmarks for Orbital plane: au_L, au_R, O_L, O_R")
    
    # Calculate auriculare midpoint
    au_midpoint = (au_L + au_R) / 2.0
    
    # Use auriculare midpoint and the two orbitale points
    points = np.array([O_R, O_L, au_midpoint])
    centroid = points.mean(axis=0)
