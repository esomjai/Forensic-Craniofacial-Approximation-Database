...
class SoftTissueThicknessPegsGUI(qt.QWidget):
    ...
    def createStep2_LoadSurface(self):
        ...
        self.normalSampleRadiusSpinBox.setRange(1, 10)
        self.normalSampleRadiusSpinBox.setValue(3)
        self.normalSampleRadiusSpinBox.setSuffix(" mm")
        self.normalSampleRadiusSpinBox.setToolTip("Radius for sampling surface normal around landmark")
    ...
    def getAnatomicalPegDirection(self, label, landmarkPos, surfaceNormal, centroid):
        """
        ...
        rightDir = np.array([-1, 0, 0])
        leftDir = np.array([1, 0, 0])
        ...
        if label == 'g':
            direction = 0.7 * anteriorDir + 0.3 * superiorDir
        ...
        if label == 'rhi':
            direction = 0.8 * anteriorDir + 0.2 * inferiorDir
            return direction / np.linalg.norm(direction)
        
        if label == 'ss':
            direction = 0.7 * anteriorDir + 0.3 * inferiorDir
            return direction / np.linalg.norm(direction)
        ...
        if label == 'gn':
            direction = 0.6 * anteriorDir + 0.4 * inferiorDir
            return direction / np.linalg.norm(direction)
        
        if label == 'me':
            direction = 0.3 * anteriorDir + 0.7 * inferiorDir
            return direction / np.linalg.norm(direction)
        ...
        if 'mso' in label.lower():
            direction = 0.7 * superiorDir + 0.3 * lateralDir
            return direction / np.linalg.norm(direction)
        
        if 'mio' in label.lower():
            direction = 0.6 * anteriorDir + 0.4 * lateralDir
            return direction / np.linalg.norm(direction)
        
        if label.startswith('ac'):
            direction = 0.6 * lateralDir + 0.4 * anteriorDir
            return direction / np.linalg.norm(direction)
        
        if label.startswith('zy'):
            direction = 0.8 * lateralDir + 0.2 * posteriorDir
            return direction / np.linalg.norm(direction)
        
        if label.startswith('sC'):
            direction = 0.5 * anteriorDir + 0.5 * lateralDir
            return direction / np.linalg.norm(direction)
        
        if label.startswith('iC'):
            direction = 0.5 * anteriorDir + 0.3 * lateralDir + 0.2 * inferiorDir
            return direction / np.linalg.norm(direction)
        
        if 'ecm2' in label.lower():
            direction = 0.7 * lateralDir + 0.3 * posteriorDir
            return direction / np.linalg.norm(direction)
        
        if label.startswith('go'):
            direction = 0.5 * lateralDir + 0.3 * inferiorDir + 0.2 * posteriorDir
            return direction / np.linalg.norm(direction)
        
        if label.startswith('mmb'):
            direction = 0.6 * lateralDir + 0.4 * inferiorDir
            return direction / np.linalg.norm(direction)
        
        if label.startswith('mr'):
            direction = 0.6 * lateralDir + 0.4 * posteriorDir
            return direction / np.linalg.norm(direction)
        
        # ========== FALLBACK ==========
        direction = 0.7 * radialDir + 0.3 * surfaceNormal
        return direction / np.linalg.norm(direction)
    ...
    def generatePegsFromModel(self):
        ...
        try:
            ...
            for i in range(numLandmarks):
                ...
                # Get anatomically correct peg direction
                pegDirection = self.getAnatomicalPegDirection(label, landmarkPos, surfaceNormal, centroid)
    ...
    def generatePegsFromVolume(self):
        ...
        try:
            ...
            for i in range(numLandmarks):
                ...
                # Get anatomically correct peg direction
                pegDirection = self.getAnatomicalPegDirection(label, landmarkPos, surfaceNormal, volumeCentroid)
    ...
    def findSurfaceNormalFromCT(self, landmarkPos, boneThresholdMin, boneThresholdMax, sampleRadius, volumeCentroid):
        """
        Simplified: Find bone surface normal from CT using gradient method.
        Returns surface-perpendicular normal.
        """
        try:
            imageData = self.volumeNode.GetImageData()
            ...
            landmarkIJK = [int(round(landmarkIJK[0])), int(round(landmarkIJK[1])), int(round(landmarkIJK[2]))]
            
            # Check bounds
            if (landmarkIJK[0] < 5 or landmarkIJK[0] >= dims[0]-5 or
                landmarkIJK[1] < 5 or landmarkIJK[1] >= dims[1]-5 or
                landmarkIJK[2] < 5 or landmarkIJK[2] >= dims[2]-5):
                print("Warning: Landmark too close to volume edge")
                return (landmarkPos - volumeCentroid) / np.linalg.norm(landmarkPos - volumeCentroid)
            ...
            for axis in range(3):
                offset = [0, 0, 0]
                offset[axis] = sampleDist
                
                i_plus = landmarkIJK[0] + offset[0]
                j_plus = landmarkIJK[1] + offset[1]
                k_plus = landmarkIJK[2] + offset[2]
                
                i_minus = landmarkIJK[0] - offset[0]
                j_minus = landmarkIJK[1] - offset[1]
                k_minus = landmarkIJK[2] - offset[2]
                ...
                gradient[axis] = (val_plus - val_minus) / (2 * sampleDist * spacing[axis])
            
            gradientMagnitude = np.linalg.norm(gradient)
            
            if gradientMagnitude < 0.001:
                print(f"Warning: Very weak gradient")
                fallback = landmarkPos - volumeCentroid
                return fallback / np.linalg.norm(fallback)
            
            # Gradient points from low to high HU, we want outward
            normal = -gradient / gradientMagnitude
            
            # Sanity check
            radialDir = landmarkPos - volumeCentroid
            radialDir = radialDir / np.linalg.norm(radialDir)
            
            dotProduct = np.dot(normal, radialDir)
            ...
            print(f"Error in findSurfaceNormalFromCT: {e}")
            fallback = landmarkPos - volumeCentroid
            return fallback / np.linalg.norm(fallback)
